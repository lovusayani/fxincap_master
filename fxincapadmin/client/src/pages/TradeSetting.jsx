...
export const TradeSetting = () => {
  ...
  }, [token, mode, status, search, from, to, page])
    ...
    <div className="flex flex-col gap-6">
      ...
      <Card title="All Trader Trades">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-4">
          ...
          {(search || mode || status || from || to) && (
            <button onClick={() => { setSearch(''); setMode(''); setStatus(''); setFrom(''); setTo(''); setPage(1) }}
        ...
        <div className="overflow-x-auto rounded-lg border border-slate-700">
          <table className="w-full border-collapse text-sm">
            <thead className="bg-slate-800/60 border-b border-slate-700">
              <tr>
                {['Trader','Symbol','Side','Mode','Volume','Open Price','Close Price','Profit','Leverage','Status','Open Time','Close Time'].map(h => (
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={12} className="px-3 py-10 text-center text-slate-400">Loading trades…</td></tr>
              ) : trades.length === 0 ? (
                <tr><td colSpan={12} className="px-3 py-10 text-center text-slate-500">No trades found</td></tr>
              ) : trades.map(t => (
                <tr key={t.id} className="border-b border-slate-800 hover:bg-slate-800/40 transition-colors">
                  ...
                  <td className="px-3 py-2.5 whitespace-nowrap">
                    <span className={`text-xs px-1.5 py-0.5 rounded ${t.mode === 'real' ? 'bg-emerald-900/40 text-emerald-400' : 'bg-sky-900/40 text-sky-400'}`}>{t.mode}</span>
                  </td>
